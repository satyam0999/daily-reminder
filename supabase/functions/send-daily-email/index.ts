...
serve(async (req) => {
  try {
    ...
    for (const userSettings of settings || []) {
        ...
        <body>
          <div class="container">
            ...
            ${dailyGoals.length > 0 ? `
              ...
              <ul>
                ${dailyGoals.map(g => `<li>${g.goal_text}</li>`).join('')}
              </ul>
            ` : ''}
            ...
            ${weeklyGoals.length > 0 ? `
              ...
              <ul>
                ${weeklyGoals.map(g => `<li>${g.goal_text}</li>`).join('')}
              </ul>
            ` : ''}
            ...
            ${monthlyGoals.length > 0 ? `
              ...
              <ul>
                ${monthlyGoals.map(g => `<li>${g.goal_text}</li>`).join('')}
              </ul>
            ` : ''}
      ...
      console.log(`SendGrid response for ${userSettings.user_email}: status=${response.status}, headers=${JSON.stringify(Object.fromEntries(response.headers.entries()))}, body=${responseText || '(empty)'}`)
      
      if (!response.ok) {
        console.error('SendGrid error:', response.status, responseText)
      } else {
        console.log(`Email queued for ${userSettings.user_email}`)
      }
    }
