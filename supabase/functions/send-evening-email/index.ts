...
async function sendEveningEmailForUser(supabase: any, userId: string, userEmail: string) {
  try {
    ...
    console.log(`Sending evening email to ${userEmail}...`)
    ...
    if (!response.ok) {
      ...
      console.log(`Evening email queued for ${userEmail}`)
    }
  } catch (error) {
    console.error(`Error sending evening email to ${userEmail}:`, error)
  }
}

serve(async (_req) => {
  try {
    console.log('Starting evening email function...')
    ...
    if (!settings || settings.length === 0) {
      ...
      for (const userId of uniqueUserIds) {
        ...
        await sendEveningEmailForUser(supabase, userId as string, authUser.user.email!)
      }
    } else {
      for (const userSettings of settings) {
        console.log(`Processing user settings for ${userSettings.user_email}`)
        await sendEveningEmailForUser(supabase, userSettings.user_id, userSettings.user_email)
      }
    }
